...
import { getPage, getSiteSettings, listAwards, listHospitals, listTeam } from "@/lib/site-cms";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: `${page.metaTitle || "About Us"} | ${settings.name}`,
...
const whyIcons = [ShieldCheck, Stethoscope, Sparkles, HeartHandshake];

export default async function AboutPage() {
  const [page, team, awards, hospitals] = await Promise.all([
    getPage("about"),
    listTeam(),
    listAwards(),
    listHospitals(),
      ...
      <section className="relative overflow-hidden bg-[#07111F] text-white">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-4 py-16 lg:grid-cols-12 lg:px-6 lg:py-24">
          ...
          <div className="grid grid-cols-2 gap-3 lg:col-span-5">
            {fields.heroImages.map((image, index) => (
              <ImageSlot
                key={`${image.src}-${index}`}
                src={image.src}
                alt={image.alt}
                label={image.label}
                className={index === 0 ? "col-span-2 min-h-[220px] lg:min-h-[260px]" : "min-h-[140px]"}
                rounded={index === 0 ? "rounded-[2rem]" : undefined}
              />
            ))}
          </div>
        </div>
      ...
      <section className="relative mx-auto max-w-7xl px-4 py-16 lg:px-6">
        <div className="grid items-center gap-10 lg:grid-cols-12">
          <div className="lg:col-span-5">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-orange">{fields.storyEyebrow}</p>
            ...
            <blockquote className="relative mt-8 rounded-[1.75rem] bg-white p-6 shadow-[0_16px_40px_-28px_rgba(15,23,42,0.4)] ring-1 ring-black/5">
              ...
              <footer className="mt-3 text-sm text-brand-muted">{fields.quoteBy}</footer>
            </blockquote>
          </div>
          <ul className="grid gap-3 sm:grid-cols-2 lg:col-span-7">
            {fields.principles.map((item) => (
              <li
                key={item.index}
      ...
      <section id="team" className="scroll-mt-24 px-4 py-16 lg:px-6">
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-orange">{fields.teamEyebrow}</p>
        ...
        <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-brand-dark px-8 py-12 text-white">
          <div className="flex flex-col items-start justify-between gap-8 lg:flex-row lg:items-center">
            <div className="max-w-xl">
              <h2 className="text-3xl font-bold tracking-tight">{fields.ctaTitle}</h2>
              <p className="mt-3 text-sm leading-7 text-slate-300">
                {fields.ctaBody}
              </p>
              <ul className="mt-5 space-y-2 text-sm text-slate-200">
                {fields.ctaBullets.map((item) => (
