      ...
      <section className="relative overflow-hidden bg-[#07111F] text-white">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-16 lg:px-6 lg:py-20">
          ...
          <p className="mt-5 max-w-xl text-base leading-8 text-slate-300">
            The path you asked for does not exist. Agents should recover from the sitemap or llms.txt instead of treating
        ...
        <ul className="mt-4 grid gap-3 sm:grid-cols-2">
          {recovery.map((item) => (
            <li key={item.href}>
              <Link
                href={item.href}
                className="block rounded-2xl bg-white p-4 text-sm font-semibold text-brand-dark ring-1 ring-black/5"
              >
                {item.label}
              </Link>
            </li>
