...
export function BrandManager({ brands }: { brands: CatalogBrand[] }) {
  ...
  return (
    <div>
      ...
      <p className="mt-2 max-w-xl text-sm text-brand-muted">
        These names appear in the model form. Brand story pages on the public site still use the marketing copy in
        the codebase.
      </p>

      <form
        onSubmit={onCreate}
        className="mt-8 grid gap-3 rounded-3xl bg-white p-6 ring-1 ring-black/5 sm:grid-cols-3"
      >
        <label>
        ...
        <label>
          ...
          <input
            className={adminField}
            placeholder="/images/brands/signia.svg"
            value={logoUrl}
        ...
        <button
          type="submit"
          disabled={pending}
          className="rounded-full bg-brand-orange px-4 py-2.5 text-sm font-semibold text-white sm:col-span-3"
        >
          {pending ? "Adding…" : "Add brand"}
        </button>
        {error ? <p className="text-sm text-brand-orange sm:col-span-3">{error}</p> : null}
      </form>

      <ul className="mt-6 grid gap-3 sm:grid-cols-2">
        {brands.map((brand) => (
          <li key={brand.id} className="flex items-center gap-4 rounded-3xl bg-white p-4 ring-1 ring-black/5">
